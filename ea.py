...
def set_p_bag_num(sum_list, p_times, bag_num_score):
    ...
    i = 0
    while i < p_times:
        bag_num_random_int = random.randint(60, 80)
        bag_list = []
...
while cycles_times < termination_times:
    ...
    p_father_dict = {}
    p_father_list = []
    p_father_dict[str(p_sum_value_dict_sort[0][0])] = p_fit_data[str(p_sum_value_dict_sort[0][0])]
    p_father_dict[str(p_sum_value_dict_sort[1][0])] = p_fit_data[str(p_sum_value_dict_sort[1][0])]

    p_father_list.append(p_fit_data[str(p_sum_value_dict_sort[0][0])])
    p_father_list.append(p_fit_data[str(p_sum_value_dict_sort[1][0])])

    p_father_dict_res = p_father_dict
    ...
    if e_capacity_sum < 285:
        if e_value_sum > p_sum_value_dict_sort[len(p_sum_value_dict_sort)-1][1]:
            p_fit_data[str(p_sum_value_dict_sort[len(p_sum_value_dict_sort)-1][0])] = e_set_list
    ...
    final_list = p_fit_data[p_sum_value_dict_sort[0][0]]
